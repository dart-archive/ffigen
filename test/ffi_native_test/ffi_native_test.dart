...
import '_expected_ffi_native_test_bindings.dart' as bindings;

void main() {
  group('native_test', () {
    setUpAll(() {
      logWarnings();
      var dylibName = 'test/ffi_native_test/native_test.so';
      if (Platform.isMacOS) {
        dylibName = 'test/ffi_native_test/native_test.dylib';
      } else if (Platform.isWindows) {
        dylibName = r'test\ffi_native_test\native_test.dll';
      }
    ...
    test('generate_bindings', () {
      final configFile =
          File(path.join('test', 'ffi_native_test', 'config.yaml')).absolute;
      final outFile = File(
        path.join('test', 'debug_generated',
            '_expected_ffi_native_test_bindings.dart'),
      ).absolute;
